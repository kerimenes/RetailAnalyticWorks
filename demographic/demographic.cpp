...
void DemographicManager::Reload() {
  ...
  for (int i = 0; i < 100; i++) {
    retail::yca::Person p;
    p.set_id(i);
    p.mutable_details()->set_age(rand() % 65);
    p.mutable_details()->set_gender((i % 2 == 0
                                         ? retail::yca::PersonDetails::MAN
                                         : retail::yca::PersonDetails::WOMAN));
